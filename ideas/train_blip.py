...
model_name = f'./vit/final'
# model_name = f'./vit1/{category}/final'
...
counts = df['attr_1'].value_counts()

# Filter out rows with count >= 100
df = df[df['attr_1'].isin(counts[counts >= 200].index)]
print(len(df))
...
def compute_metrics(pred):
    ...
    for i in range(labels.shape[1]):
        ...
        print(classification_report(labels_,probs_,target_names=list(label2id[i].keys())))
...
for layer in model.vit.encoder.layer[0:len(model.vit.encoder.layer)-2]:
